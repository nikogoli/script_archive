...
bl_info = {
    "name": "3-Buttons Shortcut",
    "author": "nikogoli",
    "version": (1, 0),
    "blender": (3, 3, 0),
    "location": "None",
    "description": "Custom 3-buttons shortcut",
    "warning": "",
    "support": "TESTING",
    "wiki_url": "",
    "tracker_url": "",
...
def reset_groups(context):
    ...
    (_keymap, km_item) = addon_keymaps[0]
    AddonPrefs._main_kmi = km_item
    
    group = groups.add()
    group.mode_name = km_item.properties.mode
    group_item = group.op_items.add()

    km_container = context.window_manager.keyconfigs.addon.keymaps.new(__name__)
    keyitem = km_container.keymap_items.new('', "A", "PRESS")	
...
class OperatorItemGroup(bpy.types.PropertyGroup):	
    """ キー入力とオペレーターの設定のまとめ

    show_expanded (bool) : 詳細表示の有無
    mode_name (str) : 現状、name 要素としての役割のみ
    op_items : OperatorItem を要素とする CollectionProperty
    """
    show_expanded: BoolProperty( name='Show Details', default=False)
    mode_name: StringProperty( name="Mode name", default="VIEW_3D" )
    ...
    def draw(self, context:Context, layout:UILayout):
        ...
        row_item1 = main_row.row()
        icon = 'TRIA_DOWN' if self.show_expanded else 'TRIA_RIGHT'
        row_item1.prop(self, "show_expanded", text='', icon=icon, emboss=False)

        main_row.label(text=self.mode_name)

        if self.show_expanded:
            indented = indented_layout(context, base, 1)
            indented.context_pointer_set('group_parent', self)
            for item in self.op_items:
                item.draw(context, indented)
            
            last_row = indented.row()
            last_row.alignment = 'LEFT'
            op = last_row.operator(WM_OT_keyitem_manipulate.bl_idname, text='Add New', icon='ADD')
            op.method = 'add_item'
...
class WM_OT_keyitem_manipulate(bpy.types.Operator):
    ...
    method (str): 行う処理. add_item | remove_item
    """
    ...
    def execute(self, context):
        ...
        if self.method == "add_item":
            ...
            i = list(group.op_items).index(group_item)
            key_items.remove(keyitem)
            group.op_items.remove(i)

        return {'FINISHED'}


class WM_OT_three_button_operator(bpy.types.Operator):
    """ モーダルモードでキー入力を監視し、入力に対応するオペレーターを実行する

    mode (enum of OperatorItemGroup'name): OperatorItemGroup の名前. 今のところ何も意味はない
    handle: handler を格納するためのもの
    """
    bl_idname = "wm.three_button_operator"
    bl_label = "Operator called by 3-buttons"
    bl_options = {'REGISTER', 'UNDO'}

    mode: StringProperty(name="mode", default="VIEW_3D")
    handle = None
    ...
    def __init__(self):
        ...
        sub_keys : OperatorItem に登録されているキー入力のリスト
        key_items (bpy.types.KeyMapItems) :  KeyMapItem の検索対象となる KeyMap の.keymap_items 要素
        prop_items (list[CustomGestureItem]):  OperatorItem のリスト
        item (bpy.types.KeyMapItem):  実行するオペレーターが登録された KeyMapItem
        prop_item (CustomGestureItem):  実行するオペレーターの情報を持つ OperatorItem
        """
        self.handle = None
        self.main_kmi: Union[KeyMapItem, None] = None
        self.sub_keys: list[str] = []
        self.key_items: Union[list[KeyMapItem], None] = None
        self.prop_items: Union[list[OperatorItem], None] = None
        self.item: Union[KeyMapItem, None] = None
        self.prop_item:Union[OperatorItem, None] = None
    ...
    def draw_handler_add(self, context):
        if WM_OT_three_button_operator.handle is None:
            WM_OT_three_button_operator.handle = bpy.types.SpaceView3D.draw_handler_add(
                WM_OT_three_button_operator.my_callback, (self, context), 'WINDOW', 'POST_PIXEL'
            )


    def draw_handler_remove(self, context):
        if WM_OT_three_button_operator.handle is not None:
            bpy.types.SpaceView3D.draw_handler_remove(
                WM_OT_three_button_operator.handle, 'WINDOW'
            )
            WM_OT_three_button_operator.handle = None
    ...
    def modal(self, context, event):
        ...
        if anyed in self.sub_keys:
            idx = self.sub_keys.index(anyed)
            self.item = self.key_items[idx]
            self.prop_item = self.prop_items[idx]

        event_str = event_to_string(event)
        if self.item == None and event_str in self.sub_keys:
            idx = self.sub_keys.index(event_str)
            self.item = self.key_items[idx]
            self.prop_item = self.prop_items[idx]

        if self.item == None or self.prop_item == None:
            self.draw_handler_remove(context)
            return {'CANCELLED'}
        else:
            split = self.item.idname.split('.')
            operator = None
            if len(split) == 2:
                m, o = split
                if m in dir(bpy.ops):
            ...
            self.draw_handler_remove(context)
            args = [self.prop_item.exec_context, True]
            kwargs = {}
            for arg in dir(self.item.properties):
                if not arg.startswith("_") and arg not in ["bl_rna", "rna_type"]:
                    kwargs[arg] = getattr(self.item.properties, arg)
    ...
    def invoke(self, context, event):
        ...
        default_sub = " ".join(key_to_string(self.main_kmi).split(" ")[:-1])

        key_items = context.window_manager.keyconfigs.addon.keymaps[__name__].keymap_items
        group = [x for x in prefs.mode_groups if x.mode_name == self.mode][0]
        self.key_items = [key_items.from_id(item.idx) for item in group.op_items]
        self.sub_keys = [ f"{default_sub} " + key_to_string(keyitem) for keyitem in self.key_items ]		
        self.prop_items = group.op_items
        context.region.tag_redraw()
        return {'RUNNING_MODAL'}
...
class AddonPrefs(bpy.types.AddonPreferences):
    ...
    mode_groups: CollectionProperty( name='Items', type=OperatorItemGroup)
    ...
    def draw(self, context):
        ...
        for group in self.mode_groups:
            row = layout.row()
            group.draw(context, row)


#---------------------------------------

classes = [
     WM_OT_three_button_operator,
...
def register():
    ...
    if kc:
        ...
        kmi = km.keymap_items.new(WM_OT_three_button_operator.bl_idname, 'Q', 'PRESS', shift=1, head=True)
